...
it('snapshot with no content matches', () => {
    const tree = renderer.create(<AfterLogin />).toJSON();
...
it('renders the page component', () => {
    ...
    expect(wrapper).toContainExactlyOneMatchingElement('Banner1');
    expect(wrapper).toContainExactlyOneMatchingElement('Footer2');
});
...
it('test', async () => {
    ...
    render(<AfterLogin url="http://test.com" />);
